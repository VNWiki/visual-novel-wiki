 ...
 * @returns {string|null} The title from frontmatter, or null if not found or error.
 */
function getFrontmatterTitle(filePath) {
  try {
    ...
    return null; // Silently fail or log a warning
  }
}
...
function getPageItemConfig(filePath, baseLinkPath, isIndexFileForGroup = false, dirNameForIndexFallback = '') {
  ...
  if (isIndexFileForGroup) {
    text = titleFromFrontmatter || nameToTitleCase(dirNameForIndexFallback);
    // For directory index files, link should end with a slash (e.g., /guide/)
    // baseLinkPath (e.g., /guide) will not have a trailing slash from its construction.
    link = `${baseLinkPath}/`;
  } else {
    text = titleFromFrontmatter || nameToTitleCase(fileNameWithoutExtension);
...
function generateSidebar() {
  ...
  // These appear before any directory-based groups.
  rootEntries
